            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:
                        wait = 0
            self.msg("Paused", dw - 150, dh - 100, blue, 40)
            pygame.display.flip()
        ...
        while wait:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:
                        wait = 0
            self.msg("Gameover", dw - 200, dh - 100, red, 40)
            self.msg("Press Enter to Play Again", dw - 530, dh + 200, red, 40)
            pygame.display.flip()
        self.new()

    def scores(self):
        self.msg("Score:" + str(self.score), dw - 130, 10, green, 30)

    def update(self):
        self.all_sprites.update()
        now = pygame.time.get_ticks()
        hits1 = pygame.sprite.spritecollide(
            self.bird, self.bblocks, False, pygame.sprite.collide_mask
        )
        hits2 = pygame.sprite.spritecollide(
            self.bird, self.tblocks, False, pygame.sprite.collide_mask
        )
        if hits1 or hits2:
            self.over()
        relx = self.bgx % bw + 5
